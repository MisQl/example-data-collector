...
public class UserParentIdProvider implements DataProvider {

    private final UserRepository userRepository;

    @Override
    public SimpleStep supports() {
        return new SimpleStep(USER_ID, USER_PARENT_ID);
    }
    ...
    public Optional<Data> get(Data source) {
        ...
        if (value instanceof Number number) {
            return userRepository.findById(number.longValue()).map(user -> new Data(USER_PARENT_ID, user.getParentId()));
        }
        return Optional.empty();
    }
}
