...
import static com.example.datacollector.core.DataField.USER_LASTNAME;

@Component
@RequiredArgsConstructor
public class UserLastnameProvider implements DataProvider {

    private final UserRepository userRepository;

    @Override
    public SimpleStep supports() {
        return new SimpleStep(USER_ID, USER_LASTNAME);
    }
    ...
    public Optional<Data> get(Data source) {
        ...
        if (value instanceof Number number) {
            return userRepository.findById(number.longValue()).map(user -> new Data(USER_LASTNAME, user.getLastname()));
        }
        return Optional.empty();
    }
}
